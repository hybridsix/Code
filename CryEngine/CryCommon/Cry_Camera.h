...
private:		
#if defined(XENON_INTRINSICS) && !defined(__SPU__)
	static XMVECTOR MinDotProductWithCross( const XMVECTOR xv[8], uint32 p0, uint32 p1,
                                          XMVECTOR cltp, XMVECTOR crtp, XMVECTOR crbp, XMVECTOR clbp );
	static Vec3_f64 GetCross( const XMVECTOR xv[8], uint32 p0, uint32 p1 );
	bool DoublePrecisionCheck( const XMVECTOR xv[8], uint32 frontx8 ) const;
#endif
...
#define SELECT_1         0xFFFFFFFF
    ...
    uint32 m_BitX[FRUSTUM_PLANES], m_BitY[FRUSTUM_PLANES], m_BitZ[FRUSTUM_PLANES];

    for (int nPl=0;nPl<FRUSTUM_PLANES;nPl++) 
    {
      f32_u ux; ux.floatVal = m_fp[nPl].n.x;
      f32_u uy; uy.floatVal = m_fp[nPl].n.y;
      f32_u uz; uz.floatVal = m_fp[nPl].n.z;
      uint32 bitX	=	ux.uintVal>>31;
      uint32 bitY	=	uy.uintVal>>31;
      uint32 bitZ	=	uz.uintVal>>31;

      m_BitX[nPl] = bitX*SELECT_1;
      m_BitY[nPl] = bitY*SELECT_1;
      m_BitZ[nPl] = bitZ*SELECT_1;
    }

    m_PlaneNX[0] = XMVectorSet(m_fp[0].n.x, m_fp[1].n.x, m_fp[2].n.x, m_fp[3].n.x);
    m_PlaneNY[0] = XMVectorSet(m_fp[0].n.y, m_fp[1].n.y, m_fp[2].n.y, m_fp[3].n.y);
    m_PlaneNZ[0] = XMVectorSet(m_fp[0].n.z, m_fp[1].n.z, m_fp[2].n.z, m_fp[3].n.z);
    m_PlaneDist[0] = XMVectorSet(m_fp[0].d, m_fp[1].d, m_fp[2].d, m_fp[3].d);

    m_idxSelX[0] = XMVectorSetInt(m_BitX[0], m_BitX[1], m_BitX[2], m_BitX[3]);
    m_idxSelY[0] = XMVectorSetInt(m_BitY[0], m_BitY[1], m_BitY[2], m_BitY[3]);
    m_idxSelZ[0] = XMVectorSetInt(m_BitZ[0], m_BitZ[1], m_BitZ[2], m_BitZ[3]);

    if (i!=2 && i!=3)
    {
      m_PlaneNX[1] = XMVectorSet(m_fp[0].n.x, m_fp[1].n.x, m_fp[4].n.x, m_fp[5].n.x);
      m_PlaneNY[1] = XMVectorSet(m_fp[0].n.y, m_fp[1].n.y, m_fp[4].n.y, m_fp[5].n.y);
      m_PlaneNZ[1] = XMVectorSet(m_fp[0].n.z, m_fp[1].n.z, m_fp[4].n.z, m_fp[5].n.z);
      m_PlaneDist[1] = XMVectorSet(m_fp[0].d, m_fp[1].d, m_fp[4].d, m_fp[5].d);

      m_idxSelX[1] = XMVectorSetInt(m_BitX[0], m_BitX[1], m_BitX[4], m_BitX[5]);
      m_idxSelY[1] = XMVectorSetInt(m_BitY[0], m_BitY[1], m_BitY[4], m_BitY[5]);
      m_idxSelZ[1] = XMVectorSetInt(m_BitZ[0], m_BitZ[1], m_BitZ[4], m_BitZ[5]);
    }
#endif
  }
...
#if defined(XENON_INTRINSICS) && !defined(__SPU__)
    m_BitX[i] = bitX*SELECT_1;
    m_BitY[i] = bitY*SELECT_1;
    m_BitZ[i] = bitZ*SELECT_1;
#endif
	}
...
#if defined(XENON_INTRINSICS) && !defined(__SPU__)
	// VMX version about 2.5 times faster on Xenon
	XMVECTOR sphere = __loadunalignedvector(&s);
	XMVECTOR cx = __vspltw(sphere, 0);
	XMVECTOR cy = __vspltw(sphere, 1);
	XMVECTOR cz = __vspltw(sphere, 2);
	XMVECTOR radius = __vspltw(sphere, 3);
	XMVECTOR mRadius = __vsubfp( XMVectorZero(), radius);

	// px, py, pz, pw are the x, y, z, w components of the planes p0, p1, p2, p3
	XMVECTOR px = m_PlaneNX[0];
	XMVECTOR py = m_PlaneNY[0];
	XMVECTOR pz = m_PlaneNZ[0];
	XMVECTOR pw = m_PlaneDist[0];

	// Calculate the first 4 distances into each component of 'd0'
	XMVECTOR d0 = __vmaddfp(pz, cz, __vmaddfp(py, cy, __vmaddfp(px, cx, pw)));

	XMVECTOR qx = m_PlaneNX[1];
	XMVECTOR qy = m_PlaneNY[1];
	XMVECTOR qz = m_PlaneNZ[1];
	XMVECTOR qw = m_PlaneDist[1];

	// Calculate the last 2 distances into each component of 'd1'
	XMVECTOR d1 = __vmaddfp(qz, cz, __vmaddfp(qy, cy, __vmaddfp(qx, cx, qw)));

	// The compare below is a greater than than so take the max
	XMVECTOR dmax = __vmaxfp( d0, d1 );

	if (vec_any_gt( dmax, radius ))
	{
		return CULL_EXCLUSION;
	}

	// Now check for inclusion
	if (vec_any_gt( dmax, mRadius ))
	{
		return CULL_OVERLAP;
	}
...
{
  const f32  * __restrict p=&aabb.min.x;
  const f32  * __restrict p3 = &p[3]; 

#ifndef PS3
  unsigned int regCmpRes;
#endif

  PrefetchLine( p, sizeof(AABB) );

  __vector4 vmxZero = (__vector4)__vspltisw(0);

  __vector4 vMin = __vor ( __lvlx( p, 0), __lvrx( p, 16) );
  __vector4 vMax = __vor ( __lvlx( p3, 0), __lvrx( p3, 16) ); //TF load only word

  __vector4 vMinX = __vspltw( vMin, 0 );
  __vector4 vMaxX = __vspltw( vMax, 0 );
  __vector4 vMinY = __vspltw( vMin, 1 );
  __vector4 vMaxY = __vspltw( vMax, 1 );
  __vector4 vMinZ = __vspltw( vMin, 2 );
  __vector4 vMaxZ = __vspltw( vMax, 2 );


  __vector4 vX = (__vector4)__vsel(vMinX, vMaxX, m_idxSelX[0]);
  __vector4 vY = (__vector4)__vsel(vMinY, vMaxY, m_idxSelY[0]);
  __vector4 vZ = (__vector4)__vsel(vMinZ, vMaxZ, m_idxSelZ[0]);
  __vector4 vDP = __vmaddfp(vX, m_PlaneNX[0], m_PlaneDist[0]);
  vDP = __vmaddfp(vY, m_PlaneNY[0], vDP);
  vDP = __vmaddfp(vZ, m_PlaneNZ[0], vDP);

#ifdef PS3
	if(vec_any_gt(vDP, vmxZero))
#else
	__vcmpgtfpR(vDP, vmxZero, &regCmpRes);
	if ((regCmpRes & (1 << 5))==0)
#endif
  {
    return CULL_EXCLUSION;
  ...
  vX = (__vector4)__vsel(vMinX, vMaxX, m_idxSelX[1]);
  vY = (__vector4)__vsel(vMinY, vMaxY, m_idxSelY[1]);
  vZ = (__vector4)__vsel(vMinZ, vMaxZ, m_idxSelZ[1]);
  vDP = __vmaddfp(vX, m_PlaneNX[1], m_PlaneDist[1]);
  vDP = __vmaddfp(vY, m_PlaneNY[1], vDP);
  vDP = __vmaddfp(vZ, m_PlaneNZ[1], vDP);
#ifdef PS3
	if(vec_any_gt(vDP, vmxZero))
#else
  __vcmpgtfpR(vDP, vmxZero, &regCmpRes);
...
//////////////////////////////////////////////////////////////////////////
  ...
  vX = (__vector4)__vsel(vMaxX, vMinX, m_idxSelX[0]);
  vY = (__vector4)__vsel(vMaxY, vMinY, m_idxSelY[0]);
  vZ = (__vector4)__vsel(vMaxZ, vMinZ, m_idxSelZ[0]);
  vDP = __vmaddfp(vX, m_PlaneNX[0], m_PlaneDist[0]);
  vDP = __vmaddfp(vY, m_PlaneNY[0], vDP);
  vDP = __vmaddfp(vZ, m_PlaneNZ[0], vDP);
  ...
  vX = (__vector4)__vsel(vMaxX, vMinX, m_idxSelX[1]);
  vY = (__vector4)__vsel(vMaxY, vMinY, m_idxSelY[1]);
  vZ = (__vector4)__vsel(vMaxZ, vMinZ, m_idxSelZ[1]);
  vDP = __vmaddfp(vX, m_PlaneNX[1], m_PlaneDist[1]);
  vDP = __vmaddfp(vY, m_PlaneNY[1], vDP);
  vDP = __vmaddfp(vZ, m_PlaneNZ[1], vDP);
#ifdef PS3
...
inline 
#endif
bool CCamera::IsAABBVisible_FM( const AABB& aabb ) const
{
...
#if defined(XENON_INTRINSICS) && !defined(__SPU__)
// VMX Optimised function for getting min dot product with cross vector
ILINE XMVECTOR CCamera::MinDotProductWithCross(
		const XMVECTOR xv[8], uint32 p0, uint32 p1,
		XMVECTOR cltp, XMVECTOR crtp, XMVECTOR crbp, XMVECTOR clbp )
{
	XMVECTOR s = XMVector3Cross( xv[p0], xv[p1] );
	XMVECTOR dotLT = __vmsum3fp( s, cltp );
	XMVECTOR dotRT = __vmsum3fp( s, crtp );
	XMVECTOR dotRB = __vmsum3fp( s, crbp );
	XMVECTOR dotLB = __vmsum3fp( s, clbp );
	XMVECTOR mins = __vminfp( __vminfp( dotLT, dotRT ), __vminfp( dotRB, dotLB ) );
	return mins;
}

// Get the cross vector (double) from a pair of XMVECTORs
ILINE Vec3_f64 CCamera::GetCross(const XMVECTOR xv[8], uint32 p0, uint32 p1)
{
	Vec3_f64 v0( (const Vec3&)xv[p0] );
	const Vec3& v1 = (const Vec3&)xv[p1];
	return v0.Cross(v1);
}

// In circumstances where the floating point precision is not enough for the algorthim we need to perform the 
// fin checks in double precision. 
inline bool CCamera::DoublePrecisionCheck(const XMVECTOR xv[8], uint32 frontx8) const
{
	...
	if(sideamount==6) {
		...
		Vec3_f64 s2( GetCross( xv, p2, p3 ) );	
		if (  (s2|m_cltp)>0 && (s2|m_crtp)>0 && (s2|m_crbp)>0 && (s2|m_clbp)>0  ) return CULL_EXCLUSION;
		Vec3_f64 s3( GetCross( xv, p3, p4 ) );	
		if (  (s3|m_cltp)>0 && (s3|m_crtp)>0 && (s3|m_crbp)>0 && (s3|m_clbp)>0  ) return CULL_EXCLUSION;
		Vec3_f64 s4( GetCross( xv, p4, p5 ) );	
		if (  (s4|m_cltp)>0 && (s4|m_crtp)>0 && (s4|m_crbp)>0 && (s4|m_clbp)>0  ) return CULL_EXCLUSION;
		Vec3_f64 s5( GetCross( xv, p5, p0 ) );	
...
{
#if defined(XENON_INTRINSICS) && !defined(__SPU__)
  const XMVECTOR zero = XMVectorZero(); // = (XMVECTOR)(0,0,0,0)
 	const XMVECTOR ione = (XMVECTOR)__vspltisw(1); // = (XMVECTOR)(1,1,1,1)
 	const XMVECTOR half = __vcfsx( ione, 1 ); // = (XMVECTOR)(0.5f,0.5f,0.5f,0.5f)
	XMVECTOR aabbMin = __loadunalignedvector(&aabb.min);
	XMVECTOR aabbMax = __loadunalignedvector(&aabb.max);
	XMVECTOR m = __vmulfp( __vaddfp(aabbMin, aabbMax), half );

	XMVECTOR mx = __vspltw( m, 0 );
	XMVECTOR my = __vspltw( m, 1 );
	XMVECTOR mz = __vspltw( m, 2 );
	...
	XMVECTOR dp0 = __vmaddfp(pz, mz, __vmaddfp(py, my, __vmaddfp(px, mx, pw)));
	...
	XMVECTOR dp1 = __vmaddfp(qz, mz, __vmaddfp(qy, my, __vmaddfp(qx, mx, qw)));

	XMVECTOR dmax = __vmaxfp( dp0, dp1 );

	if (vec_all_lt( dmax, zero ))
	{
		return CULL_OVERLAP;
	}
	// GetPosition() does some scalar which would introduce LHS penalties when converting to XMVECTOR
	Matrix34A rMatrix=m_Matrix;;
	XMVECTOR m0 = XMVec4::LoadVec4(&rMatrix.m0);
	XMVECTOR m1 = XMVec4::LoadVec4(&rMatrix.m1);
	XMVECTOR m2 = XMVec4::LoadVec4(&rMatrix.m2);
	
	XMVECTOR posxz = __vmrglw( m0, m2 );
	XMVECTOR posyw = __vmrglw( m1, zero );
	XMVECTOR campos = __vmrglw( posxz, posyw );

	XMVECTOR vmin = __vsubfp( aabbMin, campos );
	XMVECTOR vmax = __vsubfp( aabbMax, campos );

	const XMVECTOR minus1 = (XMVECTOR)__vspltisw( -1 );
	static const XMVECTORI masks = { 0x00080020, 0x00800010, 0x00400100, 0x48f42400 /* 500000.0 */ };
	
	XMVECTOR minMasks = __vupkhsh( masks );
	XMVECTOR maxMasks = __vsldoi( minMasks, __vupklsh(masks), 12 );

	XMVECTOR minSigns = __vsraw( __vsubuwm(zero, vmin), minus1 );
	XMVECTOR maxSigns = __vsraw( vmax, minus1 );

	minSigns = __vand(minMasks, minSigns);
	maxSigns = __vand(maxMasks, maxSigns);

	XMVECTOR vfrontx8 = __vor( minSigns, maxSigns );
	XMVECTOR vfrontx8yyyy = __vspltw( vfrontx8, 1 );
	XMVECTOR vfrontx8zzzz = __vspltw( vfrontx8, 2 );
	
	vfrontx8 = __vor( vfrontx8, vfrontx8yyyy );
	vfrontx8 = __vor( vfrontx8, vfrontx8zzzz );

	XMVECTOR v0FFF = __vsldoi( zero, minus1, 12 );

	if (vec_any_eq_uint( vfrontx8, v0FFF ))
	{
		return CULL_OVERLAP; //AABB is patially visible
	}

	// No choice: must pay an LHS to get the x component into a uint32
	union 
	{
		XMVECTOR 	vec;
		uint32		u;
	...
	uint32vector.vec = vfrontx8;
	uint32 frontx8 = uint32vector.u; // LHS

	const XMVECTOR signBits_xyz = (XMVECTOR)__vsldoi( (XMVECTOR)__vslw( minus1, minus1 ), zero, 4 );
	XMVECTOR absMin = __vandc( vmin, signBits_xyz );
	XMVECTOR absMax = __vandc( vmax, signBits_xyz );
	
	XMVECTOR bounds = __vmaxfp( absMin, absMax );
	XMVECTOR bounds_y = __vspltw(bounds, 1);
	XMVECTOR bounds_z = __vspltw(bounds, 2);
	XMVECTOR boundsMax = __vmaxfp( __vmaxfp(bounds, bounds_y), bounds_z );
	XMVECTOR precisionMax = __vsldoi( (XMVECTOR)masks, zero, 12 );

	const XMVECTOR sel011 = v0FFF;
	const XMVECTOR sel001 = __vsldoi( zero, minus1, 8 );
	const XMVECTOR sel010 = __vmrghw( zero, minus1 );

	XMVECTOR xv[8];
 	xv[0] = vmin;
	xv[1] = __vsel( vmax, vmin, sel011 );
	xv[2] = __vsel( vmin, vmax, sel010 );
	xv[3] = __vsel( vmax, vmin, sel001 );
	xv[4] = __vsel( vmin, vmax, sel001 );
	xv[5] = __vsel( vmax, vmin, sel010 );
	xv[6] = __vsel( vmin, vmax, sel011 );
	xv[7] = vmax;
	if (vec_all_ge( boundsMax, precisionMax ))
	{
		return DoublePrecisionCheck(xv, frontx8);
	}

	XMVECTOR cltp = __loadunalignedvector( &m_cltp );
	XMVECTOR crtp = __loadunalignedvector( &m_crtp );
	XMVECTOR crbp = __loadunalignedvector( &m_crbp );
	XMVECTOR clbp = __loadunalignedvector( &m_clbp );
	...
	if(sideamount==4) {
		...
		//-----  and clip them against the 4 side-frustum-planes of the AABB        -
		//--------------------------------------------------------------------------
		if (vec_all_gt( MinDotProductWithCross(xv, p0, p1, cltp, crtp, crbp, clbp ), zero )) return CULL_EXCLUSION;
		if (vec_all_gt( MinDotProductWithCross(xv, p1, p2, cltp, crtp, crbp, clbp ), zero )) return CULL_EXCLUSION;
		if (vec_all_gt( MinDotProductWithCross(xv, p2, p3, cltp, crtp, crbp, clbp ), zero )) return CULL_EXCLUSION;
		if (vec_all_gt( MinDotProductWithCross(xv, p3, p0, cltp, crtp, crbp, clbp ), zero )) return CULL_EXCLUSION;
	}
	else
	if(sideamount==6) {
		//--------------------------------------------------------------------------
		...
		if (vec_all_gt( MinDotProductWithCross(xv, p1, p2, cltp, crtp, crbp, clbp ), zero )) return CULL_EXCLUSION;
		if (vec_all_gt( MinDotProductWithCross(xv, p2, p3, cltp, crtp, crbp, clbp ), zero )) return CULL_EXCLUSION;
		if (vec_all_gt( MinDotProductWithCross(xv, p3, p4, cltp, crtp, crbp, clbp ), zero )) return CULL_EXCLUSION;
		if (vec_all_gt( MinDotProductWithCross(xv, p4, p5, cltp, crtp, crbp, clbp ), zero )) return CULL_EXCLUSION;
		if (vec_all_gt( MinDotProductWithCross(xv, p5, p0, cltp, crtp, crbp, clbp ), zero )) return CULL_EXCLUSION;
	}
#else	
