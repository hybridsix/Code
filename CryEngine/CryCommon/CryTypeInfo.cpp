...
const CTypeInfo::CVarInfo* CStructInfo::FindSubVar(cstr name, bool bRecurseBase) const
{
	...
	for (int i = s_nLast, iEnd = s_nLast+nSize; i < iEnd; i++)
	{
		...
		if (bRecurseBase && var.IsBaseClass())
		{
			if (const CVarInfo* pSubVar = var.Type.FindSubVar(name, true))
				return pSubVar;
		}
	}
	return 0;
	...
	if (num_elems)
	{
		...
		for (int i = 0; i < Elems.size(); i++)
		{
			...
			if (Elems[i].Name[0])
			{
				if (!*sPrefix)
				{
					sPrefix = Elems[i].Name;
					nPrefixLength = strlen(sPrefix);
				}
				else
				{
					size_t p = 0;
					while (p < nPrefixLength && Elems[i].Name[p] == sPrefix[p])
						p++;
					nPrefixLength = p;
				}
			}
